//To ensure jquery is loaded in your JavaScript application, call checkJquery(); before creating new Spokes object

maxretries = 40; // allow 4 seconds to load jQuery
retry = 0;
...
Plugin.prototype.incomingCall = function( name, callID, contact, tones, route, callback )
{
  ...
  if( callID == undefined || callID.getName() != "SpokesCallId" || !isNumber( callID.Id ) || contact == undefined || contact.getName() != "SpokesContact" || !isNumber( tones) || !isNumber( route) ){
    callback( new SpokesResponse( {isError:true, Err: {Type:2, Description:"IncomingCall invalid parameters", Error_Code:0} } ) );
    return false;
  }
...
Plugin.prototype.outgoingCall = function( name, callID, contact,  route, callback )
{
  ...
  if( callID == undefined || callID.getName() != "SpokesCallId" || !isNumber( callID.Id ) || contact == undefined || contact.getName() != "SpokesContact" || !isNumber( route)) {
		callback( new SpokesResponse( {isError:true, Err: {Type:2, Description:"OutgoingCall invalid parameters", Error_Code:0} } ) );
		return false;
  }
...
Plugin.prototype.terminateCall = function( name, callID, callback )
{
  ...
  if( callID == undefined || callID.getName() != "SpokesCallId" || !isNumber( callID.Id ) ){
  		callback( new SpokesResponse( {isError:true, Err: {Type:2, Description:"TerminateCall invalid parameters", Error_Code:0} } ) );
		return false;
  }
...
Plugin.prototype.answerCall = function( name, callID, callback )
{
    // Error handling
  if( callID == undefined || callID.getName() != "SpokesCallId" || !isNumber( callID.Id ) ){
  		callback( new SpokesResponse( {isError:true, Err: {Type:2, Description:"AnswerCall invalid parameters", Error_Code:0} } ) );
		return false;
  }
...
Plugin.prototype.holdCall = function( name, callID, callback )
{
    // Error handling
  if( callID == undefined || callID.getName() != "SpokesCallId" || !isNumber( callID.Id ) ) {
  		callback( new SpokesResponse( {isError:true, Err: {Type:2, Description:"HoldCall invalid parameters", Error_Code:0} } ) );
		return false;
  }
...
Plugin.prototype.resumeCall = function( name, callID, callback )
{
    // Error handling
  if( callID == undefined || callID.getName() != "SpokesCallId" || !isNumber( callID.Id ) ){
  		callback( new SpokesResponse( {isError:true, Err: {Type:2, Description:"ResumeCall invalid parameters", Error_Code:0} } ) );
		return false;
  }
...
Plugin.prototype.muteCall = function( name, callID, muted, callback )
{
    // Error handling
  if( callID == undefined || callID.getName() != "SpokesCallId" || !isNumber( callID.Id ) ) {
  		callback( new SpokesResponse( {isError:true, Err: {Type:2, Description:"MuteCall invalid parameters", Error_Code:0} } ) );
		return false;
  }
...
Plugin.prototype.insertCall = function( name, callID, contact, callback )
{
  ...
  if( callID == undefined || callID.getName() != "SpokesCallId" || !isNumber( callID.Id ) || contact == undefined || contact.getName() != "SpokesContact" ) {
  		callback( new SpokesResponse( {isError:true, Err: {Type:2, Description:"InsertCall invalid parameters", Error_Code:0} } ) );
		return false;
  }
...
Plugin.prototype.setAudioRoute = function( name, callID, route, callback )
{
  ...
  if( callID == undefined || callID.getName() != "SpokesCallId" || !isNumber( callID.Id ) ||!isNumber( route)) {
		callback( new SpokesResponse( {isError:true, Err: {Type:2, Description:"SetAudioRoute invalid parameters", Error_Code:0} } ) );
		return false;
  }
...
Plugin.prototype.setConferenceId = function( name, callID, callback )
{
    // Error handling
  if( callID == undefined || callID.getName() != "SpokesCallId" || !isNumber( callID.Id )){
  		callback( new SpokesResponse( {isError:true, Err: {Type:2, Description:"SetConferenceId invalid parameters", Error_Code:0} } ) );
		return false;
  }
...
Plugin.prototype.makeCall = function( name, contact, callback )
{
  ...
  if( contact == undefined || contact.getName() != "SpokesContact" ) {
  		callback( new SpokesResponse( {isError:true, Err: {Type:2, Description:"MakeCall invalid parameters", Error_Code:0} } ) );
		return false;
  }
...
// useful validation related global variables
id_to_focus = "";
validation = true;

// validate int field, use in onblur handler and also in final submit click command (submit)
// id is the html element name
// name is the user readable textual name
// min and max define min and max values
// allowempty specifies if user can leave field blank. set to true for onblur handler, and false for final click command (submit)
...
// start a timer to focus a control on the web page in 100 ms
// disable validation to avoid onblur endless loop
...
// actually focus the page element
function setTimerFocus()
{
    if (id_to_focus != "") {
        document.getElementById(id_to_focus).focus();
        document.getElementById(id_to_focus).select();
    }
    validation = true;
}